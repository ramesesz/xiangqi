...
var BOARD_SIZE_COL = 9;
var BOARD_SIZE_ROW = 10;
...
function validSquare(square) {
  ...
  return (square.search(/^[a-i][0-9]+$/) !== -1);
}

function validPieceCode(code) {
  if (typeof code !== 'string') return false;
  return (code.search(/^[rb][GAEHRCS]$/) !== -1);
}
...
function validFen(fen) {
  ...
  // number of rows 
  if (chunks.length !== BOARD_SIZE_ROW) return false;

  // check the piece sections
  for (var i = 0; i < BOARD_SIZE_ROW; i++) {
	if (chunks[i].search(/^[gaehrcsGAEHRCS1-9]*/) === -1) {
	      return false;
	}
  }
...
// convert FEN piece code to bR, wS, etc
...
// convert bR, wS, etc code to FEN structure
...
function fenToObj(fen) {
	...
	var currentRow = BOARD_SIZE_ROW-1;
...
function objToFen(obj) {
  ...
  var currentRow = BOARD_SIZE_ROW-1;
  for (var i = 0; i < BOARD_SIZE_ROW; i++) {
    for (var j = 0; j < BOARD_SIZE_COL; j++) {
...
var MINIMUM_JQUERY_VERSION = '1.7.0',
  START_FEN = 'rheagaehr/9/1c5c1/s1s1s1s1s/9/9/S1S1S1S1S/1C5C1/9/RHEAGAEHR',
...
*/
  ...
  if (cfg.hasOwnProperty('pieceTheme') !== true ||
    ...
    cfg.pieceTheme = 'img/xiangqipieces/{piece}.png';
  }
...
function calculateSquareSize() {
  ...
  while (boardWidth % BOARD_SIZE_COL !== 0 && boardWidth > 0) {
    boardWidth--;
  }

  return (boardWidth / BOARD_SIZE_COL);
}
...
function createElIds() {
  ...
  for (var i = 0; i < COLUMNS.length; i++) {
    ...
    for (var j = 0; j < BOARD_SIZE_ROW; j++) {
...
function buildBoard(orientation) {
  ...
  var html = '<div style="background-image: url(img/xiangqibackground.png); background-size:cover"; >';

  // algebraic notation / orientation
  var alpha = deepCopy(COLUMNS);
  var row = BOARD_SIZE_ROW-1;
  if (orientation === 'black') {
    alpha.reverse();
    row = 0;
  }
  ...
  for (var i = 0; i < BOARD_SIZE_ROW; i++) {
    ...
    for (var j = 0; j < BOARD_SIZE_COL; j++) {
      ...
      if (cfg.showNotation === true) {
        ...
        if ((orientation === 'white' && row === 0) ||
            (orientation === 'black' && row === BOARD_SIZE_ROW-1)) {
    ...
    html += '<div class="' + CSS.clearfix + '"></div>';
...
function buildSparePieces(color) {
  var pieces = ['rG', 'rA', 'rE', 'rH', 'rR', 'rC', 'rS'];
  if (color === 'black') {
    pieces = ['bG', 'bA', 'bE', 'bH', 'bR', 'bC', 'bS'];
  }
...
function updateFEN() {
  $('#fenout').html(objToFen(CURRENT_POSITION));	
}
...
widget.resize = function() {
  ...
  // set board width
  boardEl.css('width', (SQUARE_SIZE * BOARD_SIZE_COL) + 'px');
...
function initDom() {
  ...
  $('body').append(buildPiece('bS', true, draggedPieceId));
